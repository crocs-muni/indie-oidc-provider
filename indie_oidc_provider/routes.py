...
bp = Blueprint("indie_oidc_provider", "home")
...
def from_browser() -> bool:
    ua = request.headers.get("User-Agent")
    for browser_type in ["Mozilla", "AppleWebKit", "Chrome", "Safari"]:
        if browser_type in ua:
            return True
    return False
...
def home():
    ...
    return jsonify(
        [{"info": c.client_info, "metadata": c.client_metadata} for c in clients]
    )
